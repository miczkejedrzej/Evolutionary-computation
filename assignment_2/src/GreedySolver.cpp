...
struct Node_List
{
    ...
    void print() const
    {
        std::cout << "Node_List (capacity=" << capacity << "):\n";
        for (size_t i = 0; i < cost.size(); ++i)
        {
            std::cout << "  pos=" << position[i]
                      << " city=" << cityIndex[i]
                      << " cost=" << cost[i] << "\n";
        }
        if (cost.empty())
...
GreedySolver::GreedySolver(const ProblemInstance &prob, int start_idx, GreedyMode mode, Heuristic heuristic,
    ...
    : Solver(prob), mode_(mode), heuristic(heuristic), starting_index(start_idx), weight(weightObjective) {}
...
std::vector<int> GreedySolver::complete_solution(std::vector<int> partial_solution)
{
    int targetSize = problem.GetNumberCitiesInCycle();
    std::vector<int> visited;
    visited.reserve(targetSize);
    for (int city : partial_solution)
    {
